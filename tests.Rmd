...
# Test of k-means cluster analysis?
centers <- tibble(
  cluster = factor(1:3), 
...
CEIN_dat <- plant_glm_cov %>% 
  dplyr::select(site_id, CEIN, tsf_cat, sev, tsf, sev_tsf, tsf_cat2)

CEIN_dat$tsf_cat <- as.factor(CEIN_dat$tsf_cat)
CEIN_dat$tsf_cat2 <- as.factor(CEIN_dat$tsf_cat2)

CEIN_mod <- glm(CEIN/100 ~ sev + tsf_cat, CEIN_dat, family = gaussian)
summary(CEIN_mod)
...
# why are the estimates all negative???
CEIN_mod1 <- glm(CEIN/100 ~ tsf_cat, CEIN_dat, family = "gaussian")
summary(CEIN_mod1)

occ_plot_full2cat(CEIN_mod1)
...
CEIN_mod1 <- glm(CEIN/100 ~ sev, CEIN_dat, family = "gaussian")
summary(CEIN_mod1)
...
PSME_dat <- plant_glm_cov %>% 
  dplyr::select(site_id, PSME, tsf_cat, sev, tsf_cat2)

PSME_mod <- glm(PSME ~ sev + tsf_cat2, PSME_dat, family = "gaussian")
summary(PSME_mod)
occ_plot_full2cat(PSME_mod1)


PSME_mod1 <- glm(PSME ~ sev, PSME_dat)
summary(PSME_mod1)
...
hs1_SE <- logit2prob(CEIN_tidy[1,3])
hs2_SE <- logit2prob(CEIN_tidy[1,3]) + logit2prob(CEIN_tidy[4,3])
hs3_SE <- logit2prob(CEIN_tidy[1,3]) + logit2prob(CEIN_tidy[5,3])
ls1_SE <- logit2prob(CEIN_tidy[1,3]) + logit2prob(CEIN_tidy[2,3])
ls2_SE <- logit2prob(CEIN_tidy[1,3]) + logit2prob(CEIN_tidy[2,3]) + logit2prob(CEIN_tidy[4,3])
ls3_SE <- logit2prob(CEIN_tidy[1,3]) + logit2prob(CEIN_tidy[2,3]) + logit2prob(CEIN_tidy[5,3])
un_SE <- logit2prob(CEIN_tidy[1,3]) + logit2prob(CEIN_tidy[3,3])
...
hs1_SE <- logit2prob(CEIN_tidy[1,3])
hs2_SE <- logit2prob(CEIN_tidy[1,3]) + logit2prob(CEIN_tidy[2,3])
hs3_SE <- logit2prob(CEIN_tidy[1,3]) + logit2prob(CEIN_tidy[3,3])
ls1_SE <- logit2prob(CEIN_tidy[1,3]) + logit2prob(CEIN_tidy[4,3])
ls2_SE <- logit2prob(CEIN_tidy[1,3]) + logit2prob(CEIN_tidy[5,3])
ls3_SE <- logit2prob(CEIN_tidy[1,3]) + logit2prob(CEIN_tidy[6,3])
un_SE <- logit2prob(CEIN_tidy[1,3]) + logit2prob(CEIN_tidy[7,3])
...
CEIN_tidy <- tidy(mod)
# I think there should only be 2 age categories for ls and hs rather than 3
hs_mean <- logit2prob(CEIN_tidy[1,2])
ls_mean <- logit2prob(CEIN_tidy[1,2]) + logit2prob(CEIN_tidy[2,2])
un_mean <- logit2prob(CEIN_tidy[1,2]) + logit2prob(CEIN_tidy[3,2])

mean <- unlist(c(hs_mean, ls_mean, un_mean))

hs_SE <- logit2prob(CEIN_tidy[1,3])
ls_SE <- logit2prob(CEIN_tidy[1,3]) + logit2prob(CEIN_tidy[2,3])
un_SE <- logit2prob(CEIN_tidy[1,3]) + logit2prob(CEIN_tidy[3,3])
...
SE1 <- logit2prob(CEIN_tidy[1,3])
SE2 <- logit2prob(CEIN_tidy[1,3]) + logit2prob(CEIN_tidy[2,3])
SE3 <- logit2prob(CEIN_tidy[1,3]) + logit2prob(CEIN_tidy[3,3])
un_SE <- logit2prob(CEIN_tidy[1,3]) + logit2prob(CEIN_tidy[4,3])
...
sp_dat <- plant_glm_cov %>% 
  dplyr::select(site_id, ACMA, tsf_cat, sev, tsf, sev_tsf)
...
```{r ANMAD}
sp_dat <- plant_glm_cov %>% 
  dplyr::select(site_id, ANMAD, tsf_cat, sev, tsf, sev_tsf)

lmt(sp_dat)

sp_mod1 <- glm(ANMAD/100 ~ sev, sp_dat, family = gaussian)
...
```{r ARME}
sp_dat <- plant_glm_cov %>% 
  dplyr::select(site_id, ARME, tsf_cat, sev, tsf, sev_tsf)
...
```{r CEIN}
sp_dat <- plant_glm_cov %>% 
  dplyr::select(site_id, CEIN, tsf_cat, sev, tsf, sev_tsf)

lmt(sp_dat)

sp_mod1 <- glm(CEIN/100 ~ sev + tsf_cat, sp_dat, family = gaussian)
...
sp_dat <- plant_glm_pa %>% 
  dplyr::select(site_id, CONU, tsf_cat, sev, tsf, tsf_cat2, sev_tsf)
...
```{r NODE}
sp_dat <- plant_glm_cov %>% 
  dplyr::select(site_id, NODE, tsf_cat, sev, tsf, tsf_cat2)
...
```{r PSME}
sp_dat <- plant_glm_cov %>% 
  dplyr::select(site_id, PSME, tsf_cat, sev, tsf, tsf_cat2)
