...
#this is the way to calculate AICc
library(bbmle)
AICctab(MECA_mod,MECA_mod1, base=T, delta=T, weights=T)
...
# get r squared values, but need to get them in the right order. use a conditional process?
#ifelse(attr(sp_aic, "row.names")[1] == "sev", sev, ifelse(attr(sp_aic, "row.names")[1] == "tsf", tsf, full))

#sp_R2_1 <- rsq(ifelse(attr(sp_aic, "row.names")[1] == "sev", sev, ifelse(attr(sp_aic, "row.names")[1] == "tsf", tsf, full)), adj = T)

#sp_R2_1 <- rsq(attr(sp_aic, "row.names")[1], adj = T)
#sp_R2_2 <- rsq(attr(sp_aic, "row.names")[2], adj = T)
#sp_R2_3 <- rsq(attr(sp_aic, "row.names")[3], adj = T)
...
#sp_tsfcat_R2 <- rsq(tsf, adj = T)
#sp_sev_R2 <- rsq(sev, adj = T)
#sp_mod_R2 <- rsq(full, adj = T)


#sp_full <-  glm(log1p(sp_dat[,2]) ~ sev*tsf_cat, sp_dat, family = "gaussian")
#sp_tsfcat <- glm(log1p(sp_dat[,2]) ~ tsf_cat, sp_dat, family = "gaussian")
#sp_sev <- glm(log1p(sp_dat[,2]) ~ sev, sp_dat, family = "gaussian")
#sp_null  <-glm(log1p(sp_dat[,2]) ~ 1, sp_dat, family = "gaussian")

#but check the QQ plot: log1p or logit transformation is way better.
#plot(CEIN_sev)
#plot(CEIN_full)
#CEIN_sev_t <- glm(log1p(CEIN) ~ sev, cein_dat, family = "gaussian")
#CEIN_aic <- AICctab(CEIN_full,CEIN_full_t, base=T, delta=T, weights=T)
#lrtest(CEIN_sev, CEIN_sev_t)

#how to get the right lrt into the table with the right aic? Same with the R2 for that matter
...
# Now the table BUT: the order in the aic changes depending on which model is best; the way the table is shown it throws that data into a pre existing order, so that's wrong
...
fitstats <- function(sp_dat, ind) {
  fm <- glm(sp_dat[,2]/100 ~ sev + tsf_cat, sp_dat[ind,], family = "binomial")
  observed <- fm$y
  expected <- fitted(fm)
  resids <- residuals(fm)
  sse <- sum(resids^2)
