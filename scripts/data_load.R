...
site_data <- site_data_read %>% 
  dplyr::mutate(tsf_cat = ifelse(is.na(tsf), "4", "3")) %>% 
  dplyr::mutate(tsf_2cat = ifelse(is.na(tsf), "3", "2"))
...
site_data <- site_data_read %>% 
  dplyr::mutate(tsf_cat = ifelse(is.na(tsf), "5", "4")) %>% 
  dplyr::mutate(tsf_2cat = ifelse(is.na(tsf), "3", "2"))
...
site_data <- site_data %>% 
  dplyr::mutate(sev_tsf = paste(sev, tsf_cat, sep = "-"))  %>% 
  dplyr::mutate(sev_tsf2 = paste(sev, tsf_2cat, sep = "-")) %>%
  dplyr::mutate(burn = ifelse(sev == "u", 0, 1))

d2rad <- function(deg) {(deg *pi) / 180}

site_data <- site_data %>% 
  dplyr::mutate(folded_aspect180 = 180 - abs(aspect - 180)) %>%
  dplyr::mutate(folded_aspect = 180 - abs(aspect - 225)) %>%
  dplyr::mutate(lat_rad = d2rad(lat)) %>% 
  dplyr::mutate(slope_rad = d2rad(slope)) %>% 
  dplyr::mutate(fold_asp_rad = d2rad(folded_aspect)) %>% 
  dplyr::mutate(heat_load = 0.339 + 0.808 * cos(lat_rad) * cos(slope_rad) - 0.196 * sin(lat_rad) * sin(slope_rad) - 0.482 * cos(fold_asp_rad) * sin(slope_rad))

site_data$site_id <- as.character(site_data$site_id)

site_data$sev <- as.character(site_data$sev)

site_data <- site_data[-(111:114),] %>%# remove the LICH sites 
dplyr::mutate(cov_cat = ifelse(tree_cov > 65, 3, ifelse(tree_cov > 30, 2, 1))) %>% # creating tree cover categories   
dplyr::mutate(sev_cov = ifelse(sev == "u", "u", ifelse(sev == "h", "h", ifelse(sev =="Rx", paste("RX/mult", cov_cat, sep = "-"), ifelse(sev == "multiple", paste("RX/mult", cov_cat, sep = "-"),  ifelse(sev == "multiple/RX", paste("RX/mult", cov_cat, sep = "-"), paste(sev, cov_cat, sep = "-"))))))) %>% 
  dplyr::mutate(tsf = ifelse(sev == "u", 100, tsf)) %>% 
  dplyr::mutate(sev = ifelse(tree_cov <= 25, "h", sev)) %>%  # this moves 36, 261, 260, 213, 258 from mult to HS
#  dplyr::filter(tree_cov <= 25 & sev == "multiple") %>% 
  dplyr::mutate(sev3 = ifelse(sev == "u" | sev == "l", "lu", sev)) %>% 
  dplyr::mutate(tsf_num = ifelse(tsf == "NA", "100", tsf))
#dplyr::mutate(sev_cov = paste(sev, cov_cat, sep = "-")) %>% 
...
plant_dat <- left_join(plant_data, plant_names, by = "species") %>%
  ...
  dplyr::mutate(species = ifelse(species == "CASC", "ASPR", species))  %>% 
  dplyr::mutate(species = ifelse(species == "DIHO", "PRHO", species))  %>% 
  dplyr::mutate(species = ifelse(species == "DIIM", "DIID", species))  %>% #these three are mistakes
#  dplyr::mutate(species = full_name)  %>% 
  dplyr::mutate(species = ifelse(genus == "Lupinus", "Lupinus", species)) %>% 
  dplyr::mutate(species = ifelse(genus == "Arctostaphylos", "Arctostaphylos", species)) %>% 
  dplyr::mutate(species = ifelse(genus == "Penstemon", "Penstemon", species)) %>% 
  dplyr::mutate(species = ifelse(genus == "Sidalcea", "Sidalcea", species)) %>% 
  dplyr::mutate(species = ifelse(genus == "Iris", "Iris", species)) %>% 
  dplyr::mutate(species = ifelse(genus == "Nemophila", "Nemophila", species)) %>% 
  dplyr::mutate(species = ifelse(genus == "Agoseris", "Agoseris", species)) %>% 
  dplyr::mutate(species = ifelse(genus == "Collinsia", "Collinsia", species)) %>% 
  dplyr::mutate(species = ifelse(genus == "Clarkia", "Clarkia", species)) %>% 
  dplyr::mutate(species = ifelse(genus == "Pyrola", "Pyrola", species)) %>% 
  dplyr::mutate(species = ifelse(genus == "Ribes", "Ribes", species)) %>% 
  dplyr::mutate(species = ifelse(genus == "Piperia", "Piperia", species)) %>% 
  dplyr::mutate(species = ifelse(genus == "Platanthera", "Piperia", species)) %>% 
  dplyr::mutate(species = ifelse(genus == "Madia", "Madia", species)) %>% 
  dplyr::mutate(species = ifelse(genus == "Cirsium", "Cirsium", species))  %>% 
  dplyr::mutate(species = ifelse(genus == "Silene", "Silene", species))  %>% 
  dplyr::mutate(species = ifelse(genus == "Castilleja", "Castilleja", species))  %>% 
  dplyr::mutate(species = ifelse(genus == "Cryptantha", "Cryptantha", species))  %>% 
  dplyr::mutate(species = ifelse(genus == "Dichelostemma", "Dichelostemma", species))  %>% 
  dplyr::mutate(species = ifelse(genus == "Triteleia", "Triteleia", species))  %>% 
...
#  dplyr::mutate("DUMB" = "0.5") 

## adding back in the site id and removing true/false row 
#(and making sure it's all read in as numeric)
#plant_mat_cov_w <- plant_matrix[-111, ]  #for only 2018 data, use change 111 to 49; this used to be plant_dat_cov, same as plant_mrpp_d;  %>% dplyr::mutate_if(is.character, as.numeric)
...
lichen_mat_species_w <- read.csv("data/lichen_data.csv", header = T) %>% 
  ...
  replace(is.na(.), 0)# %>% 
#  dplyr::mutate("DUMB" = 1)  # adding dummy species (eg. Webster 2010) 

#lichen_dat_species_w <- merge(lichen_mat_species_w, site_data, by = "site_id")

#species level analysis
lichen_dat_species_pa_l <- lichen_mat_species_w %>% 
  dplyr::mutate(MECA = ifelse(MECA > 0, MECA, ifelse(MEXX > 0, MEXX, 0))) %>% 
  dplyr::mutate(ALSA = ifelse(ALSA > 0, ALSA, ifelse(ALXX > 0, ALXX, 0)))  %>% 
  dplyr::mutate(USXX = ifelse(USXX > 0, USXX, ifelse(USTU > 0, USTU, ifelse(USPE > 0, USPE, ifelse(USCA > 0, USCA, ifelse(USSC > 0, USSC, 0)))))) %>% 
  dplyr::mutate(XAHA = ifelse(XAHA > 0, XAHA, ifelse(POXX > 0, POXX, 0))) %>% 
  ...
  dplyr::mutate(full_name = ifelse(genus == "Usnea", "Usnea sp.", as.character(full_name))) %>% 
  dplyr::mutate(full_name = ifelse(species == "NONE", "NONE", as.character(full_name))) %>%   
  dplyr::select(site_id,full_name,abund) %>% 
  dplyr::rename(species = full_name) %>% 
  dplyr::mutate(pa = ifelse(abund >0, 1, 0)) %>% 
...
lichen_dat_genus_l <- lichen_mat_species_w %>% 
  ...
  gather(key=species, value=abund, AHPA:XAHA) %>% 
  left_join(., lichen_names, by = "species") %>% 
  dplyr::mutate(species = ifelse(species == "MEXX", "MECA", species)) %>% 
  dplyr::mutate(species = ifelse(species == "ALXX", "ALSA", species))  %>% 
  dplyr::mutate(species = ifelse(species == "USTU", "USXX", species))  %>% 
  dplyr::mutate(species = ifelse(species == "USPE", "USXX", species))  %>% 
  dplyr::mutate(species = ifelse(species == "POXX", "XAHA", species))  %>% 
  dplyr::select(site_id,full_name,abund)  


# remove duplicate rows (same species detected multiple times in a single plot)
#lichen_dat_genus_pa1 <- lichen_dat_genus_l %>% 
#  dplyr::mutate(pa = ifelse(abund == 0, 0, 1)) %>% 
...
insect_dat1 <- read.csv("data/insect_data.csv", header = T) %>% 
  dplyr::mutate(order = as.character(order)) %>% 
  dplyr::mutate(suborder = as.character(suborder))
...
#  filter(family != "Curculionidae" & family != "Formicidae" & order != "Arachnida", order != "Apterygota" & order != "Collembola" & order != "Myriapoda") %>% # here, can decide to look at only pan, or only cones
  ...
  dplyr::mutate(trap_id = paste(site_id, trap)) %>% 
  dplyr::mutate(taxon = ifelse(suborder %in% "", order, suborder)) # use suborder when available
...
#  dplyr::rename(site_id = Point) %>% 
#  dplyr::rename(species = Spp) %>% 
  dplyr::mutate(species = recode(species, BTYW='BTYW/HEWA', RUHU='XXHU', HEWA='BTYW/HEWA')) %>%  #changing all RUHU and ANHU to XXHU; BTYW and HEWA to BTYW/HEWA
#  dplyr::dplyr::mutate(species = ifelse(species %in% c("BTYW", "HEWA"), "BTYW/HEWA", ifelse(species %in% c("RUHU", "ANHU"), "XXHU", species))) %>% 
