...
#edna data
plant_list <- read.csv("data/plant_list.csv")

plant_edna <- merge(plant_dat, plant_list, by= 'species') %>% filter(site_id == 28|site_id == 52|site_id == 34|site_id == 47|site_id == 51|site_id == 40|site_id == 1|site_id == 53|site_id == 6|site_id == 49|site_id == 25|site_id == 41) %>% select(site_id,full_name, species, cover) %>% arrange(site_id)

write.csv(plant_edna, "data/plants_edna.csv")

# Now the GLM
bin.glm1 = glm(pa~sev*tsf,data=plant_glm,family=binomial)
...
# Now the GLM
bin.glm1 = glm(pa~sev*tsf,data=MAGR_glm,family=binomial)
...
# Now the GLM
bin.glm1 = glm(pa~sev*tsf,data=PSME_glm,family=binomial)
