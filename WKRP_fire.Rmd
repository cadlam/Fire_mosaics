...
ggplot(all_spp_div1, aes(x = sev3, y = richness)) +
  geom_boxplot()

ggplot(all_spp_div1, aes(x = sev3, y = shannon)) +
  geom_boxplot()

ggplot(all_spp_div1, aes(x = sev3, y = simpson)) +
  geom_boxplot()
...
ggplot(bird_div1, aes(x = sev, y = richness)) +
  geom_boxplot()

ggplot(bird_div1, aes(x = sev, y = shannon)) +
  geom_boxplot()

ggplot(bird_div1, aes(x = sev, y = simpson)) +
  geom_boxplot()
...
ggplot(lichen_div1, aes(x = sev, y = richness)) +
  geom_boxplot()
...
## Gamma level diversity
```{r}
fsattr = fsTable %>% select(Regen_Plot_Master, OBS_FIRE_SEV) %>% rename("Regen_ID" = "Regen_Plot_Master")

gamma.matrix = all_spp_dat_l %>%
#  select(-site_id) %>%
  group_by(site_id) %>%
  summarise(sums=sum(pa))
...
gamma.attr = gamma.matrix %>% select(sev)
gamma.abund = gamma.matrix %>% select(-sev) 
...
comp.attr = all_spp_dat_w %>% select(site_id)
comp.abund = all_spp_dat_w %>% select(c(ABCO:YRWA)) 

richA = specnumber(comp.abund)
shanA = exp(diversity(comp.abund, index = "shannon"))
simpA = diversity(comp.abund, index = "invsimpson")

alpha.stats = data.frame(comp.attr, richA[,2], shanA[,2], simpA[,2])
...
alldiv = left_join(alpha.stats, site_data, by = "site_id") %>% left_join(gamma.stats) %>% 
