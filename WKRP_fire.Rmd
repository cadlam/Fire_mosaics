...
# Birds
bird_matrix <- bird_dat_w %>% mutate(sev3 = ifelse(sev == "u" | sev == "l", "lu", sev)) %>% dplyr::select(sev3,ACWO:YRWA)
...
# Plants
plant_matrix <- plant_dat_pa_w %>% mutate(sev3 = ifelse(sev == "u" | sev == "l", "lu", sev)) %>% dplyr::select(sev3,ABCO:YAMI)
...
# Lichens
lichen_matrix <- lichen_dat_species_w %>% mutate(sev3 = ifelse(sev == "u" | sev == "l", "lu", sev)) %>% dplyr::select(sev3,AHPA:XAHA)
...
mod_add_plant = lm(dis~ sev3, data = bj_output_plant)
anova(mod_add_plant)
means = emmeans(mod_add_plant, pairwise~sev3)
