...
cbp1 <- c("#1B9E77", "#8B5742", "#7570B3", "#E7298A", "#D95F02")
...
ggplot(site_data, aes(x = sev, y = tree_cov)) +
  ...
  ggtitle("Fig.1: Canopy cover in each severity category") +
  ylab("canopy cover (%)")  +
  xlab("Habitat")  +
...
pair_birds <- format(pairwise.adonis((comp.sub), comp.data1$sev2) %>% 
  dplyr::select(pairs, F.Model, p.adjusted, sig), digits = 3)
...
PERMANOVA_sev_lichens <- kable(format(adonis2(comp.sub ~ sev, permutations = 999), caption = "Table 1: PERMANOVA results"), digits = 3) %>%
  kable_styling() 
...
comp.data <- plant_dat_cov_w #%>% filter(sev != "u")
#    filter(sev == "multiple")
...
PERMANOVA_sev_plants <- kable(adonis2(log1p(comp.sub) ~ tsf + sev, permutations = 999), caption = "Table 1: PERMANOVA results (plants, cover)", digits = 5) %>%
  kable_styling()
...
comp.data1 <- comp.data %>% 
    mutate(sev2 = ifelse(sev == 'h' , "1", ifelse(sev == 'l' , "2", ifelse(sev == 'u', "3", ifelse(sev == 'multiple', "4", "5")))))# %>% 
#  mutate(sev_tsf3 = paste(sev, ".",tsf_cat))

pair_plants <- format(pairwise.adonis(log1p(comp.sub), comp.data1$sev2) %>%  #comp.data1$sev2
  dplyr::select(pairs, F.Model, p.adjusted), digits = 3)
...
### All alpha plots
#(except insects)
grid.arrange(bird_alpha, plant_alpha, lich_alpha, nrow = 1, bottom = "Fig.2: Bird, plant and lichen species richness")
```
...
```{r beta_vegetarian, echo = FALSE, fig.width=8, fig.height=6}
#library(vegetarian)
...
#Jaccard only
grid.arrange(b_BD_jaccard, p_BD_jaccard, l_BD_jaccard, nrow = 1, bottom = "Fig.3B: Bird, plant and lichen beta diversity (Jaccard)")
```
...
```{r iNEXT, echo = FALSE, fig.width=8, fig.height=6}
...
out2 <- iNEXT(plant_inext, q=c(0,1), datatype="incidence_raw", se=T, endpoint = 34) # 34 is 2x the smallest number of sites per category

plant_gamma <- ggiNEXT(out2, facet.var="order", type=1, se = TRUE) + 
    facet_wrap(~order, scales = "free_y", nrow = 1,
                strip.position = "left", 
                labeller = as_labeller(c("0" = "Species richness (\u00B1 95% CI)", "1" = "Alpha diversity (\u00B1 95% CI)") ) ) + #change to Shannon Alpha?
  ylab(NULL) +
  theme(strip.background = element_blank(),
           strip.placement = "outside") +
  xlab("Number of sites") + theme(panel.background = element_rect(fill = "white",
                                colour = "black",
                                size = 0.5, linetype = "solid")) + labs( title = "Plant gamma diversity", subtitle = "") + theme(legend.position = "none")
...
out2 <- iNEXT(bird_inext, q=c(0,1), datatype="incidence_raw", se=T, endpoint = 34) 

bird_gamma <- ggiNEXT(out2, facet.var="order", type=1, se = TRUE) +
    facet_wrap(~order, scales = "free_y", nrow = 1,
                strip.position = "left", 
                labeller = as_labeller(c("0" = "Species richness (\u00B1 95% CI)", "1" = "Alpha diversity (\u00B1 95% CI)") ) ) +
  ylab(NULL) +
  theme(strip.background = element_blank(),
           strip.placement = "outside") +
  xlab("Number of sites") + theme(panel.background = element_rect(fill = "white",
                                colour = "black",
                                size = 0.5, linetype = "solid")) + labs( title = "Bird gamma diversity", subtitle = "") + theme(legend.position = "none")
...
out2 <- iNEXT(lichen_inext, q=c(0,1), datatype="incidence_raw", se=T, endpoint = 34)

lichen_gamma <-ggiNEXT(out2, facet.var="order", type=1, se = TRUE) + #theme_bw()
  facet_wrap(~order, scales = "free_y", nrow = 1,
                strip.position = "left", 
                labeller = as_labeller(c("0" = "Species richness (\u00B1 95% CI)", "1" = "Alpha diversity (\u00B1 95% CI)") ) ) +
  ylab(NULL) +
  ...
  labs( title = "Lichen gamma diversity", subtitle = "")
