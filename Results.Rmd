...
cbp2 <- c("#D95F02", "#8B5742", "#E7298A", "#7570B3", "#1B9E77")
...
## Here, as below, the results from permutational anova are identical to regular anova.
## therefore, I still use the cld letters from anova for the graph
#out2<- aovp(tree_cov ~ sev, div)
#anova(out2) # permutational anova
#cov_hsd <- TukeyHSD(out2) # pairwise comparisons
...
cov_cld <- as.data.frame(cld.emmGrid(means$emmeans, Letters = letters))

ggplot(site_data, aes(x = sev, y = tree_cov)) +
  geom_boxplot(aes(fill=sev)) +
  ggtitle("Fig.2: Canopy cover in each fire history category") +
  ylab("canopy cover (%)")  +
  xlab("Habitat")  +
...
#cld(means$emmeans, Letters = letters)
#cwd_cld <- as.data.frame(cld.emmGrid(means$emmeans, Letters = letters))
```
...
#kable(all_pairwise_table, digits = 2, caption = "Table 2: Pairwise PERMANOVA results") %>% 
#  kable_styling("hover", full_width = F) %>%
#  add_header_above(c(" ", "plants" = 2, "birds" = 2, "lichens" = 2)) %>% 
#  save_kable("output/pair_perm.png")
```

![](output/pairwise_permanova.png)

## Variation in community composition across habitats

### Effect of fire history on species composition
The main PERMANOVA revealed that species composition varies for all taxa in response to the fire history category (*p* < 0.001). Using pairwise PERMANOVAs to compare the habitats, there is no detectable difference between plant and lichen communities of low-severity and unburnt stands, while the difference in bird communities was only marginally significant (Table 1). In addition, bird communities of high-severity burns and multiple burns are not detectably different. For lichens, contrary to expectations, the communities of multiple burns, thin+burn and low severity differed significantly. 

```{r NMDS, include = F, eval = F, echo=FALSE,results='hide'}
...
# Using lmPerm, I find the same results as using lm. Therefore, for ease, I still use lm/emmeans because of the cld () function that creates the significance letters. 

out_lich <- aovp(rich_lichen ~ sev, div)
#anova(out1) # permutational anova
lich_hsd <- TukeyHSD(out_lich) # pairwise comparisons

out_plant <- aovp(rich_plant ~ sev, div)
#anova(out1) # permutational anova
plant_hsd <- TukeyHSD(out_plant) # pairwise comparisons

out_bird <- aovp(rich_bird ~ sev, div)
#anova(out1) # permutational anova
bird_hsd <- TukeyHSD(out_bird) # pairwise comparisons

#shapiro.test(div$rich_plant) #NORMAL
#shapiro.test(div$rich_lichen) #NOT NORMAL
#ggdensity(div$rich_plant, main = "Density plot", xlab = "")
#hist(div$rich_plant)

mod_add_tsf = lm(rich_bird ~ tsf_cat, data = div %>% dplyr::filter(sev != "u"))
#anova(mod_add_tsf)
means = emmeans(mod_add_tsf, pairwise~tsf_cat)
#cld.emmGrid(means$emmeans, Letters = letters)
alpha_rich_stats_all <- as.data.frame(cld.emmGrid(means$emmeans, Letters = letters))
...
mod_add_bird = lm(sqrt(rich_bird) ~ sev, data = div)
#mod_add_bird = lm(rich_bird ~ sev, data = div)
#anova(mod_add_bird)
means = emmeans(mod_add_bird, pairwise~sev)
#cld.emmGrid(means$emmeans, Letters = letters)
alpha_rich_stats_bird <- as.data.frame(cld.emmGrid(means$emmeans, Letters = letters))
...
#cld(means$emmeans, Letters = letters)
alpha_rich_stats_plant <- as.data.frame(cld.emmGrid(means$emmeans, Letters = letters))
...
#cld(means$emmeans, Letters = letters)
alpha_rich_stats_lichen <- as.data.frame(cld.emmGrid(means$emmeans, Letters = letters))
```
...
#shapiro.test(multrx$rich_plant)
#normal

multrx_mod <- lm(sqrt(rich_plant) ~ tree_cov, multrx)
...
#shapiro.test(multrx_b$rich_bird)
#Normal

multrx_b_mod <- lm(sqrt(rich_bird) ~ tree_cov, multrx_b)
#multrx_b_anova <- anova(multrx_b_mod)
#ggplot(multrx_b, aes(x=tree_cov, y = rich_bird)) + geom_point() + geom_smooth(method = lm)
...
#lu_p1 <- lu_p %>% dplyr::filter(site_id != 25 & site_id != 36)
shapiro.test(sqrt(lu_p$rich_plant)) # NOT NORMAL

#hist(lu_b$rich_plant)

#ggdensity(lu_b$rich_plant, main = "Density plot", xlab = "")



lu_p_mod <- lm(sqrt(rich_plant) ~ tree_cov, lu_p)
...
#shapiro.test(lu_p$rich_bird) 
#normal
lu_b_mod <- lm(sqrt(rich_bird) ~ tree_cov, lu_b)
#lu_b_anova <- anova(lu_b_mod)
#ggplot(lu_b, aes(x=tree_cov, y = rich_plant)) + geom_point() + geom_smooth(method = lm)
...
#shapiro.test(hs$rich_plant) 
#normal

hs_p_mod <- lm(sqrt(rich_plant) ~ tree_cov, hs)
...
#shapiro.test(hs$rich_bird) 

hs_b_mod <- lm(sqrt(rich_bird) ~ tree_cov, hs)
...
bird_gamma <- ggiNEXT(out2, type=1, se = TRUE) +
  ...
  theme(legend.position = "none") +
  theme(aspect.ratio=1.2) + 
  scale_color_manual(values = cbp2) #+geom_line(size=0.5)
...
lichen_gamma1 <- ggiNEXT(out2, type=1, se = TRUE) + #theme_bw()
  ...
  scale_fill_manual(values = cbp2) +
  scale_color_manual(values = cbp2) +
  guides(colour = guide_legend("Fire history categories"),
  shape = guide_legend("Fire history categories"),
  fill = guide_legend("Fire history categories")) + 
  theme(aspect.ratio=1.2) +   
...
#kable(insect_indic, caption = "Table 2: Insect indicators of early-seral habitat") %>% 
#  kable_styling() %>% 
...
```{r bar_chart_all, echo = FALSE, include =F}
...
UNLS_pref_bird <- dat_percent1 %>% 
  filter(sev_ratio >= 2) %>% 
  dplyr::select(species)

UNLS_mult_bird <- dat_percent1 %>%   
  dplyr::mutate(mult_ratio = MULT_occ/HS_occ) %>% 
  filter(sev_ratio >= 2) %>% 
  filter(mult_ratio >= 2) %>% 
  dplyr::select(species)
  
UNLS_Rx_bird <- dat_percent1 %>% 
  dplyr::mutate(Rx_ratio = Rx_occ/HS_occ) %>% 
  filter(sev_ratio >= 2) %>% 
  filter(Rx_ratio >= 2) %>% 
  dplyr::select(species)
...
# Species that prefer HS
## needed for Venn diagram
HS_pref_bird <- dat_percent1 %>% 
  filter(sev_ratio <= 0.5) %>% 
  dplyr::select(species)

HS_mult_bird <- dat_percent1 %>% 
  dplyr::mutate(mult_ratio = MULT_occ/UNLS_occ) %>% 
  filter(sev_ratio <= 0.5) %>% 
  filter(mult_ratio >= 2) %>% 
  dplyr::select(species)

HS_Rx_bird <- dat_percent1 %>% 
  dplyr::mutate(Rx_ratio = Rx_occ/UNLS_occ) %>% 
  filter(sev_ratio <= 0.5) %>% 
  filter(Rx_ratio >= 2) %>% 
  dplyr::select(species)
...
I identified `r prefs_all_HS[1,3]` species (birds, plants and lichens combined) that seemed to favor high-severity burns (occurring at least twice as frequently in that habitat compared to low-severity and unburnt stands), and `r prefs_all_UNLS[1,3]` species that favored low-severity and unburnt stands (occurring at least twice as frequently in that habitat compared to high-severity burns). These habitat associations correspond to the findings of other fire ecology studies [@fontaineBirdCommunitiesFollowing2009]. Additionally, most species that have a preference for either of these habitats exhibited the same affinity for multiple burns and thinned and burnt sites: `r prefs_all_UNLS[2,3]` species (`r round(prefs_all_UNLS[2,3]/prefs_all_UNLS[1,3]*100)`%) preferring low-severity and unburnt stands also favor multiple burns, while `r prefs_all_UNLS[3,3]` (`r round(prefs_all_UNLS[3,3]/prefs_all_UNLS[1,3]*100)`%) also favor thinned and burnt sites. Conversely, `r prefs_all_HS[2,3]` species (`r round(prefs_all_HS[2,3]/prefs_all_HS[1,3]*100)`%) that prefer high-severity burns also favor multiple burns, and `r prefs_all_HS[3,3]` (`r round(prefs_all_HS[3,3]/prefs_all_HS[1,3]*100)`%) also favor thinned and burnt sites. Figure 7 displays graphically the proportion of plants and birds that prefer high-severity burns or unburnt and low-severity stands that also favor multiple burns and thin+burn sites. 
...
```{r echo = F,}
#mult_bird <- rbind(UNLS_mult_bird, HS_mult_bird)
#Rx_bird <- rbind(UNLS_Rx_bird, HS_Rx_bird)
...
#x = list(UNLS_pref_bird$species, HS_pref_bird$species, mult_bird$species, Rx_bird$species)
#VennRaw <- Venn(x)

#birds
ppp <- ggplot() +
  geom_rect(aes(xmin = -14, xmax = 0, ymin = 0, ymax = 5), color = HS_col, fill= HS_col, alpha = .5) +
  geom_rect(aes(xmin = 0, xmax = 10, ymin = 0, ymax = 5), fill = UNLS_col, alpha = .5) +
  geom_rect(aes(xmin = -14, xmax = 7, ymin = 0, ymax = 2.5), fill = mult_col) +
  geom_rect(aes(xmin = -10, xmax = 9, ymin = 2.5, ymax = 5), fill = Rx_col) +
  #geom_rect(aes(xmin = 0, xmax = 10, ymin = 2.5, ymax = 5), fill = Rx_col) +
#  geom_rect(aes(xmin = 0, xmax = 14, ymin = 0, ymax = 2.55), fill = mult_col) +
  geom_rect(aes(xmin = -14, xmax = 0, ymin = 0, ymax = 5), color = HS_col, fill= NA) +
#  geom_rect(aes(xmin = -5, xmax = 6, ymin = 0, ymax = 5), fill = "slateblue") +
#  geom_segment(aes(x=-10, y=0, xend = -10, yend = 5), color = UNLS_col) +
#  geom_segment(aes(x=-9, y=0, xend = -9, yend = 5), color = Rx_col) +
#  geom_segment(aes(x=-7, y=0, xend = -7, yend = 5), color = mult_col) +
#  geom_segment(aes(x=10, y=0, xend = 10, yend = 5), color = Rx_col) +
#  geom_segment(aes(x=14, y=0, xend = 14, yend = 5), color = HS_col) +
  geom_rect(aes(xmin = 0, xmax = 10, ymin = 0, ymax = 5), color = UNLS_col, fill= NA, alpha = .5) +
  geom_rect(aes(xmin = -14, xmax = 0, ymin = 0, ymax = 5), color = HS_col, fill= NA, alpha = .5) +
  geom_segment(aes(x=-0, y=0, xend = -0, yend = 5), color = "black") +
  theme_cowplot(12) + theme(aspect.ratio=0.5) + theme(axis.title.y=element_blank(),
        axis.text.y=element_blank(),
        axis.ticks.y=element_blank()) + 
  scale_x_continuous(name="Number of Species", breaks = seq(-15, 10, by = 5))

#plants
pp<- ggplot() +
  geom_rect(aes(xmin = -44, xmax = 0, ymin = 0, ymax = 5), color = HS_col, fill= HS_col, alpha = .5) +
  geom_rect(aes(xmin = 0, xmax = 14, ymin = 0, ymax = 5), fill = UNLS_col, alpha = .5) +
  geom_rect(aes(xmin = -34, xmax = 10, ymin = 0, ymax = 2.5, fill="Multiple"), fill = mult_col) +
  geom_rect(aes(xmin = -30, xmax = 10, ymin = 2.5, ymax = 5), fill = Rx_col) +
  #geom_rect(aes(xmin = 0, xmax = 10, ymin = 2.5, ymax = 5), fill = Rx_col) +
#  geom_rect(aes(xmin = 0, xmax = 14, ymin = 0, ymax = 2.55), fill = mult_col) +
#  geom_rect(aes(xmin = 0, xmax = 44, ymin = 0, ymax = 5), color = HS_col, fill= NA) +
#  geom_rect(aes(xmin = -5, xmax = 6, ymin = 0, ymax = 5), fill = "slateblue") +
#  geom_segment(aes(x=-10, y=0, xend = -10, yend = 5), color = UNLS_col) +
#  geom_segment(aes(x=-9, y=0, xend = -9, yend = 5), color = Rx_col) +
#  geom_segment(aes(x=-7, y=0, xend = -7, yend = 5), color = mult_col) +
#  geom_segment(aes(x=10, y=0, xend = 10, yend = 5), color = Rx_col) +
#  geom_segment(aes(x=14, y=0, xend = 14, yend = 5), color = HS_col) +
  geom_rect(aes(xmin = 0, xmax = 14, ymin = 0, ymax = 5), color = UNLS_col, fill= NA, alpha = .5) +
  geom_rect(aes(xmin = -44, xmax = 0, ymin = 0, ymax = 5), color = HS_col, fill= NA, alpha = .5) +
  geom_segment(aes(x=-0, y=0, xend = -0, yend = 5), color = "black") +
  theme_cowplot(12) +
  theme(aspect.ratio=0.5) + theme(axis.title.y=element_blank(),
        axis.text.y=element_blank(),
        axis.ticks.y=element_blank()) + 
  scale_x_continuous(name="Number of Species", breaks = seq(-45, 15, by = 15)) +
  scale_fill_manual('Multiple',
                      values = mult_col,
                      labels = "mult")
# guide = guide_legend(override.aes = list(alpha = 0.5))) 

#remove y axis
#change x axis to show only specific species numbers
# add x axis name
