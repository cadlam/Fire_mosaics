...
data_scores <- data_scores %>%
  ...
  mutate(tsf_cat2 = ifelse(tsf < 10 , "1", ifelse(tsf < 20, "2", ifelse(tsf != "3", "4"))))
```
...
comp.sub <- subset(comp.data, sev != "h" | sev == "h", select = ACTR:WHMO)
comp.env <- subset(comp.data, sev != "h" | sev == "h", select = c(fire:tsf_cat))
attach(comp.env)
