...
comptable <- all_spp_w_pa %>% full_join(site_data[, c("site_id", "sev")], by = "site_id") %>% select(-site_id) #puttig together the plot id and sev and joinign wide format data then dropping site id

betajaccard <- function(df){
  
  tempdf = df %>% select(-sev)
  
  dis = c(vegdist(tempdf, method = "jaccard", binary = "true"))
  
  result = data.frame(sev = rep(unique(df$sev), length = length(dis)),
#                      year = rep(unique(df$year), length = length(dis)),
                      dis = dis)
  
}

bj_output = comptable %>% group_by(sev) %>% do(betajaccard(.))

bj_output %>% ggplot(aes(x = sev,
                         y = dis,
                         fill = sev,
                         group = sev)) +
  geom_boxplot() +
  ...
  ggtitle("Pairwise Jaccard Beta Diversity")
