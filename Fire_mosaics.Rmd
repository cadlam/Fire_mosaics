...
pacman::p_load(tidyverse, emmeans, ggplot2, cowplot, vegan, data.table, kableExtra, plyr, nloptr, labdsv, betapart, lattice, rowr)
...
```{r}
# Calculating the average cover in each trt
plant_glm_cov1 <- plant_glm_cov %>% 
  dplyr::select(site_id:WOFI)

#plant_glm_cov1$site_id <- as.numeric(as.character(plant_glm_cov1$site_id))

#plant_glm_cov1 <- rbind(plant_glm_cov1, colSums(plant_glm_cov1[-1,])) %>%
#  select_if(function(.) last(.) >=5) # keep only species with at least 20% cumulative cover

#plant_glm_cov1$site_id <- as.factor(plant_glm_cov1$site_id)

plant_mat_UNLS <- plant_glm_cov1 %>%
  left_join(site_data, by = "site_id") %>% 
  filter(sev!="h") %>% 
  dplyr::select(ACMA:WHMO)

plant_mat_HS <- plant_glm_cov1 %>% 
  left_join(site_data, by = "site_id") %>% 
  filter(sev=="h") %>% 
  dplyr::select(ACMA:WHMO)
...
tree_ht <- read.csv("/Users/christopheradlam/Desktop/Davis/R/GitHub Repos/Fire_mosaics/Data/sublayer_data.csv")
tree_ht$site_id <- as.factor(tree_ht$site_id)

site_data_HS <- site_data %>% 
  left_join(tree_ht, by = "site_id") %>% 
  filter(sev == "h")

# use aggregate to create new data frame with the maxima
# regr_ht is the tallest tree that grew/regrew after fire
tree_ht_HS <- aggregate(regr_ht ~ site_id, site_data_HS, max) %>% 
  left_join(site_data, by= "site_id")
# then simply merge with the original
#site_data_HS <- merge(df.agg, site_data_HS1)


ggplot(tree_ht_HS, aes(x=tsf, y = regr_ht)) +
  geom_point() +
...
### Rarefaction curves (maybe div index better though)
```{r}
# rarefy only works with counts
data(BCI)
BCI <- plant_glm_pa %>% 
#  filter(sev == "u") %>% 
  dplyr::select(ABCO:WOFI) %>% 
  mutate(sev_tsf2 = paste("sev", "tsf_cat2"))

#Shannon
diversity(BCI, index = "shannon", MARGIN = 1, base = exp(1))

H <- diversity(BCI)

plant_dat_pa_h <- plant_glm_pa %>% 
  mutate(H = H) %>% 
  mutate(sev_tsf2 = paste(sev, tsf_cat2))

ggplot(plant_dat_pa_h, aes(x= sev_tsf2, y = H)) +
  geom_boxplot()
...
df <- data_summary(plant_plot, varname="num_spp", groupnames="tsf_cat2")
df
...
lichen_dat_totals <- lichen_dat_totals1[-c(1:48),-59] %>% 
...
```{r echo = F}
site_data$site_id <- as.factor(site_data$site_id)
plant_matrix1$site_id <- as.factor(plant_matrix1$site_id)

plant_mrpp_d <- left_join(plant_matrix1, site_data, by = "site_id") %>% 
  mutate(sev_tsf = paste(sev, tsf_cat, sep = "-"))

plant.mrpp <- mrpp(plant_mrpp_d[,c(2:139)], plant_mrpp_d$sev_tsf)
...
insect.mrpp <- mrpp(insect_dat_wide[,c(29:45)], insect_dat_wide$sev_tsf)

# Save and change plotting parameters
def.par <- par(no.readonly = TRUE)
layout(matrix(1:2,nr=1))

plot(insect.ord <- metaMDS(insect_dat_wide[,c(29:45)]), type="text", display="sites" )
ordihull(insect.ord, insect_dat_wide$sev_tsf, label = T)
...
## meandist 
# calculates a matrix of mean within-cluster dissimilarities (diagonal) and between-cluster dissimilarities (off-diagonal elements)
insect.md <- meandist(vegdist(insect_dat_wide[,c(29:45)]), insect_dat_wide[["sev_tsf"]])
insect.md
summary(insect.md)
plot(insect.md)
