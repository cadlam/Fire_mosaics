...
plant_dat <- left_join(plant_data, plant_names, by = "species") %>%
  ...
  dplyr::select(site_id, species, cover, obs_id)
...
plant_matrix <- plant_matrix1 %>%
  bind_rows(., plant_matrix2[111,]) %>% #for only 2018 data, use change 111 to 49
  mutate("DUMB" = "0.5") 
...
NMDS.log<- (plant_dat_pa_w[,(2:203)]) #previously tried log1p transformation for 2018, but with both years the stress is too high
...
scrs <- merge(scrs, site_data[,-(111:114)], by = "site_id")
...
#            size = 3, colour = "red") +
  labs(title = "Figure 1: NMDS for all native plants (1&2)")
...
p3d <- plot_ly(scrs, x = ~NMDS1, y = ~NMDS2, z = ~NMDS3, color = ~sev, colors = c("#FF0000", "#3399FF", "#FFFF00", "#FF9933", "#993300", "#33FF00")) %>%
  add_markers() %>%
