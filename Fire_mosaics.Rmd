...
plant_mat_cov_w <- plant_dat_cov_w %>% 
...
plant_mat_UNLS <- plant_mat_cov_w %>%
  left_join(site_data, by = "site_id") %>% 
  filter(sev!="h") %>% 
  dplyr::select(ACMA:WHMO)

plant_mat_HS <- plant_mat_cov_w %>% 
  left_join(site_data, by = "site_id") %>% 
  filter(sev=="h") %>% 
  dplyr::select(ACMA:WHMO)
...
plot_ly(color = ~sev, showlegend = F) %>%
  add_markers(data = sol3, x = ~MDS1, y = ~MDS2, z = ~MDS3) %>%
  add_paths(data = m, x = ~MDS1, y = ~MDS2, z = ~MDS3)
...
ggplot(plant_plot, aes(x = tsf, y = num_spp, color = sev)) +
  geom_boxplot(aes(group = tsf_cat)) +
  geom_jitter() +
  labs(title = "Figure X: spp num in each habitat", y = "number of spp", x = "TSF")
...
### MRPP (plants)
```{r echo = F}
plant.mrpp <- mrpp(plant_dat_cov_w[,c(2:139)], plant_dat_cov_w$sev_tsf)

# Save and change plotting parameters
def.par <- par(no.readonly = TRUE)
layout(matrix(1:2,nr=1))

plot(plant.ord <- metaMDS(plant_dat_cov_w[,c(2:139)]), type="text", display="sites" )
...
plant.md <- meandist(vegdist(plant_dat_cov_w[,c(2:139)]), plant_dat_cov_w$sev_tsf)
plant.md
summary(plant.md)
plot(plant.md)
```
