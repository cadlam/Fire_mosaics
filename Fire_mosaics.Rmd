...
mite.ordisurf.ggplot <- ggplot(mite.NMDS.data, aes(x = NMDS1, y = NMDS2))+
  ...
  geom_point(size = 3, alpha = 0.8,  aes(shape = tsf_cat)) + #plots the NMDS points, with shape by topo type
  theme_bw() + #for aesthetics
  scale_shape_manual("Topo Type",  values = c(1,16, 8, 12)) + #sets the name of the legend for shape, and says which symbols we want (equivalent to the pch command in plot)
  labs(colour = "tree cover")+ #another way to set the labels, in this case, for the colour legend
...
mite.env <- site_data %>% dplyr::select(site_id, tree_cov, tsf_cat) 

mite.env$tsf_cat <- as.factor(mite.env$tsf_cat)

#NMDS
meta.nmds.mite <- sol #metaMDS(mite) #no transformation of species data is made here prior to bray curtis dissimilarities being calculated. (Bray Curtis is the default in R).
...
#ordisurf:
ordi <- ordisurf(meta.nmds.mite ~ mite.env$tree_cov) #created the ordisurf object
...
#legend(title = "tsf_cat", "bottomleft", levels(mite.NMDS.data$tsf_cat), pch = c(1,16))
ordisurf(meta.nmds.mite~mite.env$tree_cov, col = "grey50", add = TRUE)
