...
pacman::p_load(tidyverse, emmeans, ggplot2, cowplot, vegan, data.table, kableExtra, plyr, nloptr, labdsv)
...
# Insects
insect_data <-read.csv("/Users/christopheradlam/Desktop/Davis/R/GitHub Repos/Fire_mosaics/Data/insect_data.csv")
...
ef<-envfit(sol, NMDS.log, perm=9999)
...
scrs_filt <- scrs %>% 
  dplyr::select(site_id, folded_aspect, elev, heat_load, tree_cov, shrub_cov, bare_cov, herb_cov)

sol2 <-metaMDS((scrs_filt[,-1]))

fit <- envfit(sol2, scrs_filt[,-1], na.rm = T, perm = 999)
...
data("mite")
ordi <- ordisurf(sol ~ scrs_filt$folded_aspect) #created the ordisurf object
ordi.grid <- ordi$grid #extracts the ordisurf object
str(ordi.grid) #it's a list though - cannot be plotted as is
ordi.plant <- expand.grid(x = ordi.grid$x, y = ordi.grid$y) #get x and ys
ordi.plant$z <- as.vector(ordi.grid$z) #unravel the matrix for the z scores
ordi.plant.na <- data.frame(na.omit(ordi.plant)) #gets rid of the nas
str(ordi.plant.na) #looks ready for plotting!
...
p <- ggplot(ordi.plant.na) +
  stat_contour(aes(x, y, z = z)) +
  scale_colour_gradientn(high = "darkgreen", low = "darkolivegreen1") 

?scale_color_gradient
+
  geom_point(data = scrs, mapping = aes(x = NMDS1, y = NMDS2, colour = sev)) +
  ...
  geom_text_repel(data = C1, aes(x = NMDS1, y = NMDS2, label = Env_factors),
            size = 3, colour = "red") +
  labs(title = "Figure 1: NMDS for all native plants")

p

  
  geom_point(mapping = aes(x = NMDS1, y = NMDS2, colour = sev)) +
  geom_contour(data = ordi.plant.na, aes(x = ordi.plant.na$x, y = ordi.plant.na$y, z = ordi.plant.na$z, colour = rev(..level..)), binwidth = 5)
p

  
  coord_fixed() + ## need aspect ratio of 1!
  geom_point(data = Cred, aes(x = NMDS1, y = NMDS2), shape = 3) +
...
#for genus:
NMDS.log<-log1p(lichen_dat_genus_pa[,-1])#[,-c(1:20)])
sol <- metaMDS(NMDS.log)

scrs <- as.data.frame(scores(sol, display = "sites"))
scrs <- cbind(scrs, site_id = lichen_dat_wide$site_id)
scrs <- merge(scrs, site_data, by = "site_id")
...
# but this drop all the other columns
#insect_dat2$site_id <- insect_dat3$site_id
#insect_dat2$order <- insect_dat3$order
#insect_dat2$V1 <- insect_dat3$V1
...
insect_dat$site_id <- as.character(insect_dat$site_id)

# presence/absence is not meaningful here
#insect_dat_order <- insect_dat %>% 
#  spread(key = order, value = V1, fill = 0)
...
### MRPP
```{r echo = F}
plant_mrpp_d <- left_join(plant_matrix1, site_data, by = "site_id") %>% 
  mutate(sev_tsf = paste(sev, tsf_cat, sep = "-"))

plant.mrpp <- mrpp(plant_mrpp_d[,c(1:139)], plant_mrpp_d$sev_tsf)

# Save and change plotting parameters
def.par <- par(no.readonly = TRUE)
layout(matrix(1:2,nr=1))

plot(plant.ord <- metaMDS(plant_mrpp_d[,c(2:139)]), type="text", display="sites" )
ordihull(plant.ord, plant_mrpp_d$sev_tsf, label = T)
...
plant.md <- meandist(vegdist(plant_mrpp_d[,c(2:139)]), plant_mrpp_d$sev_tsf)
plant.md
summary(plant.md)
plot(plant.md)
```
...
```{r echo = F}
insect_mrpp_d <- left_join(insect_dat, site_data, by = "site_id") %>% 
  mutate(sev_tsf = paste(sev, tsf_cat, sep = "-"))

insect.mrpp <- mrpp(insect_mrpp_d[,c(3:19)], insect_mrpp_d$sev_tsf)

# Save and change plotting parameters
def.par <- par(no.readonly = TRUE)
layout(matrix(1:2,nr=1))

plot(insect.ord <- metaMDS(insect_mrpp_d[,c(3:19)]), type="text", display="sites" )
ordihull(insect.ord, insect_mrpp_d$sev_tsf, label = T)
...
## meandist 
# calculates a matrix of mean within-cluster dissimilarities (diagonal) and between-cluster dissimilarities (off-diagonal elements)
insect.md <- meandist(vegdist(insect_mrpp_d[,c(3:19)]), insect_mrpp_d$sev_tsf)
insect.md
summary(insect.md)
plot(insect.md)
```
...
kable(pair, caption = "Table 3: pairwise PERMANOVA results", digits = 2) %>%
  kable_styling()
...
insect_dat <- left_join(insect_data, insect_names, by = "species") %>%
  filter(native_status == "native" & (form == "herb" | form == "grass")) %>%
  dplyr::select(site_id, species, cover)
insect_matrix <- spread(data = insect_dat, key = species, value = cover, fill = 0)
